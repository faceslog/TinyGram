...
public class PostTransformer implements EntityTransformer<PostEntity> {
    ...
    @Override
    public ResourceResponse<EntityResponse<PostEntity>> transformTo(PostEntity entity) {
        final EntityResponse<PostEntity> entityResponse = new BasePostResponse(entity);

        final ResourceResponse<EntityResponse<PostEntity>> resourceResponse = new ResourceResponse<>(entityResponse);
